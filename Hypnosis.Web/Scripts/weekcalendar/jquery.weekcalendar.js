		...
		return {
			options: {
				...
				*/
				freeBusyRender: function (freeBusy, $freeBusy, calendar) {
					...
					if (freeBusy.type) {
						$freeBusy.addClass(freeBusy.type);
					}
					return $freeBusy;
				},
				/* other options */
			...
			***********************/
			_create: function () {
				...
				var hour = self.options.date.getHours();
				if(hour==0) hour=self.options.businessHours.start;
				self._scrollToHour(hour, true);
			...
			*/
			setDaysToShow: function (daysToShow) {
				...
				//todo: 
				//hour = self.options.date.getHours();
			...
			*/
			_renderCalendarButtons: function ($calendarContainer) {
				...
				if (options.buttons) {
					...
					calendarNavHtml += '<input type="hidden" id="goto" name="goto" class="goto"/>';
					calendarNavHtml += '<button class=\"wc-prev\">' + options.buttonText.lastWeek + '</button>';
					calendarNavHtml += '<button class=\"wc-goto\"><i class="fa fa-calendar-o" title="' + options.buttonText.gotoDate + '"></i></button>';
					...
					calendarNavHtml += '</div>';
					calendarNavHtml += '<div class=\"wc-legend\"></div>';
              ...
              .click(function () {
              	...
              	return false;
					...
					var $date = $calendarContainer.find('input#goto');
					$date.datepicker({
						onSelect: function () {
							var selDate = $date.datepicker('getDate');
							self.element.weekCalendar('gotoDate', selDate);
						}
					});
					$calendarContainer.find('.wc-nav .wc-goto').off().click(function () {
						$date.datepicker('show');
						return false;
					});
					//now add legend
					if (this.options.legend) {
						var $container = $calendarContainer.find('.wc-legend');
						if ($.isFunction(this.options.legend))
							$container.html(this.options.legend());
						else {
							$container.html(this.options.legend);
						}
					}
			...
			*/
			_renderCalendarHeader: function ($calendarContainer) {
				var self = this, options = this.options,
            showAsSeparatedUser = options.showAsSeparateUsers && options.users && options.users.length > 1,
              ...
              _headerClass = '';
					...
					for (var i = 1; i <= options.daysToShow; i++) {
						for (var j = 0; j < uLength; j++) {
							...
							var userId = self._getUserIdFromIndex(j);
							var user = self.options.users[j];

							if (options.headerHtml) {
								var newDate = new Date(self.options.date);
								newDate.setDate(newDate.getDate() + i - 1);
								calendarHeaderHtml += options.headerHtml(user, newDate);
							}
							else {
				...
				$calendarHeader.appendTo($calendarContainer);
				if (self.options.calendarHeaderAfterRender && $.isFunction(self.options.calendarHeaderAfterRender)) {
					self.options.calendarHeaderAfterRender(self);
				}
			},
          ...
          $('<div>', {
			...
			*/
			_getCalendarTitle: function () {
				if ($.isFunction(this.options.title)) {
					return this.options.title(this.options.daysToShow, this);
				}
			...
			_renderEvents: function (data, $weekDayColumns) {
				...
				$.each(eventsToRender, function (i, calEvent) {
					...
					while (startDate < endDate) {
						...
						calEvent.end = new Date(calEvent.start.getFullYear(), calEvent.start.getMonth(), calEvent.start.getDate(), maxHour, 0, 0)
			...
			*/
			...
			_adjustOverlappingEvents: function ($weekDay) {
				...
				var user = this.getUserForId(userId);


				if (self.options.allowCalEventOverlap && user.inventory > 1) {
		 ...
		 this.options.deletable(calEvent, $calEvent)) {
			...
			*/
			_scrollToHour: function (hour, animate) {
				...
				var scroll = options.businessHours.start * options.timeslotHeight * options.timeslotsPerHour;
				var scrollOffset = targetOffset - $scrollable.offset().top;
				//console.log(self.options.date);
				//console.log("scrolling to " + hour + "hour, " + scroll + "px (slot:" + targetOffset + ", scrollable: " + $scrollable.offset().top);
			...
			* http://jacwright.com/projects/javaScripts/date_format
			*/
			...
			*/
			_getCurrentScrollHour: function () {
				...
				return Math.round(scroll / (options.timeslotHeight * options.timeslotsPerHour));
			},
			...
			_getHeaderDate: function (date) {
				...
				var dateString = dayName + (options.headerSeparator) + this._formatDate(date, options.dateFormat);
				if (options.getHeaderDate && $.isFunction(options.getHeaderDate)) {
					return options.getHeaderDate(date, this.element, dateString);
				}
				else {
					return dateString;
				}
