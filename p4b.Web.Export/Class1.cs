...
namespace OpenXmlPowerTools.SpreadsheetWriter
{
	...
	public static class SpreadsheetWriter
	{
		...
		{
			bool inherit = true;
			var workSheet = ToWorkSheet<T>(data, inherit);
		...
		public static Worksheet ToWorkSheet<T>(IEnumerable<T> data, bool inherit)
		{
			var props = typeof(T).GetProperties().Select(f => new
			{
				property = f,
				pt = Nullable.GetUnderlyingType(f.PropertyType) ?? f.PropertyType,
				d = f.GetCustomAttributes(typeof(System.ComponentModel.DataAnnotations.DisplayAttribute), inherit).Cast<System.ComponentModel.DataAnnotations.DisplayAttribute>().FirstOrDefault(),
				s = f.GetCustomAttributes(typeof(System.ComponentModel.DataAnnotations.ScaffoldColumnAttribute), inherit).Cast<System.ComponentModel.DataAnnotations.ScaffoldColumnAttribute>().FirstOrDefault(),
				f = f.GetCustomAttributes(typeof(System.ComponentModel.DataAnnotations.DisplayFormatAttribute), inherit).Cast<System.ComponentModel.DataAnnotations.DisplayFormatAttribute>().FirstOrDefault(),
				dt = f.GetCustomAttributes(typeof(p4b.Web.Export.ExcelDataTypeAttribute), inherit).Cast<p4b.Web.Export.ExcelDataTypeAttribute>().FirstOrDefault(),
			})
			.Where(f => f.s == null ? true : f.s.Scaffold)
			.Select(f => new
			{
				p = f.property,
				Title = f.d == null ? f.property.Name : f.d.GetShortName(),
				Order = f.d == null ? 0 : f.d.GetOrder(),
				d = f.d,
				f = f.f,
				cellType = f.dt == null ? type(f.pt) : f.dt.DataType


			}).OrderBy(f => f.Order).ToList();
			var propsLength = props.Count;

			return new Worksheet
			{
				Name = "Sheet1",
				ColumnHeadings = props.Select(c => new Cell
				{
					Value = c.Title,
					CellDataType = CellDataType.String,
					Bold = true
				}),
				Rows = data.Select(r => new Row
				{
					Cells = props.Select(c => new Cell
					{
						Value = c.p.GetValue(r, null),
						CellDataType = c.cellType
					})
				})
			};
		}
		private static CellDataType type(Type t)
		{
			...
			if (t == typeof(int))
				...
				return CellDataType.Number;
			else if (t == typeof(bool)) return CellDataType.Boolean;
			else if (t == typeof(DateTime)) return CellDataType.Date;
			else return
				CellDataType.String;
		}
		...
		public static int GetCellStyle(SpreadsheetDocument sDoc, Cell cell)
		{
			...
			int newId = CreateNewStyle(sXDoc, cell);
			return newId;
		}

		public static int CreateNewStyle(XDocument sXDoc, Cell cell)
		{
